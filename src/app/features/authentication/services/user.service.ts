...
import { USER_ENDPOINTS } from '~core/constants/endpoints.constants';
...
export class UserService {
  ...
  getMe(options?: { cache: boolean }): Observable<User> {
    ...
    return this.httpClient
      .get<GetMeResponse>(USER_ENDPOINTS.V1.user, {
  ...
  updateUser(updateUserRequest: UpdateUserRequest): Observable<User> {
    const updateUserEndpoint = USER_ENDPOINTS.V1.user;
  ...
  catchPokemon(catchPokemonRequest: CatchPokemonRequest): Observable<User> {
    const catchPokemonEndpoint = USER_ENDPOINTS.V1.userPokemonCatch;
    return this.httpClient
